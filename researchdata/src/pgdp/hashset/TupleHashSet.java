...
public class TupleHashSet<T, S> implements Iterable {
	...
	class TupleIterator implements Iterator<Tuple<T, S>> {
		int current = 0;

		public boolean hasNext() {
			if (current < hashArr.length && hashArr[current] != null) {
				return true;
			}
			return false;
		}
		...
		public Tuple<T, S> next() {
			Tuple<T, S> tuple = hashArr[++current];
			if (tuple == null) {
